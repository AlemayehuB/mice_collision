...
float pos[MAXPNT][3];
float vel[MAXPNT][3];
//float k = (powf(plum_v.vel[0],2.0) + powf(plum_v.vel[1],2.0) + powf(plum_v.vel[0],2.0))*0.5;
//float u = -1.0*mass/rad*powf((1 + ((powf(plum_v.pos[0],2.0) + powf(plum_v.pos[1],2.0) + powf(plum_v.pos[0],2.0))/powf(rad,2))),-0.5)
...
int nstep;					/* current value of time    */
{
 ...
 float g(float q){
   //velocity probability distributiion
   return (powf(q,2.0)*(powf((1 -powf(q,2.0)),3.5)));
 }
