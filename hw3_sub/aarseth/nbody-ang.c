...
int main()
{
    ...
    inputFile = fopen("aarseth/my_init", "r");
    // inputFile = fopen("aarseth/my_10k", "r");
    // inputFile = fopen("aarseth/my_100", "r");
    // inputFile = fopen("aarseth/coll_t0", "r");
    // inputFile = fopen("aarseth/my_20k", "r");
    readParameters();
    readParticles();
...
void outputData()
{
    double totalEnergy = 0.0, totalMass = 0.0, ke = 0.0, pe = 0.0;
    double centerOfMass[kNumDims], totalMomentum[kNumDims],
        totalAngularMomentum[kNumDims];

double refPos[kNumDims], temp[kNumDims];

    double f2Dot[kNumDims], velocity[kNumDims], deltaPos[kNumDims], dx,dy,dz;
    ...
    for(i=0; i<numParticles; i++)
    {
        ...
        ke += 0.5 * masses[i] * lengthSquared(velocity);
...
//        cross(totalAngularMomentum, positions[i], masses[i]*velocity)
    ...
    for(i=0; i<numParticles; i++)
    {
        ...
        //     pe -= masses[i]*masses[j] / sqrt(lengthSquared(deltaPos) + epsilonSquared);
        // }
        for(j=0; j<i; j++)
        {
            ...
            pe -= masses[i]*masses[j] / sqrt(dx*dx + dy*dy + dz*dz);
