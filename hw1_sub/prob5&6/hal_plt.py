...
data = pylab.loadtxt("hal.dat")
steps = round(len(data))
...
axes.scatter(x_0[:int(steps)],y_0[:int(steps)],label ="Halley")
...
axes.scatter(x_1[:int(steps/10)], y_1[:int(steps/10)],label ="Earth")


Sun = pylab.Circle((0,0), radius = 0.1, fc='y', label ='Sun')
axes.add_patch(Sun)
# axes.set_xlim([-65,45])
# axes.set_ylim([-45,45])
axes.legend()
axes.set_title("Halley & Earth Planet Orbits")
